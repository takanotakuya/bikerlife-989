...
RSpec.describe '写真テキスト編集', type: :system do
  ...
  context '写真テキスト編集ができるとき' do
    it 'ログインしたユーザーは自分が投稿した写真テキストの編集ができる' do
      ...
      # 添付する画像を定義する
      image_path = Rails.root.join('public/soreiyu.jpg')
      # 投稿内容を編集する
      attach_file('post[images][]', image_path, make_visible: true)
      fill_in 'post[name]', with: "#{@post1}+編集したテキスト"
      ...
      expect(page).to have_selector("img[src$='soreiyu.jpg']")
      # 投稿写真一覧トページには先ほど変更した内容の写真テキストが存在することを確認する（テキスト）
      expect(page).to have_content("#{@post1}+編集したテキスト")
    end
  end
...
RSpec.describe '写真テキスト削除', type: :system do
  ...
  context '写真テキスト削除ができるとき' do
    it 'ログインしたユーザーは自らが投稿した写真テキスト削除ができる' do
      ...
      expect(page).to have_no_selector("img[src$='#{@post1}test_image.png']")
